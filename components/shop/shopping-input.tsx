...
export function ShoppingInput({ onResearch, loading }: { onResearch: (prompt: string) => void; loading?: boolean }) {
  const [value, setValue] = useState("");
  const [recent, setRecent] = useState(["Gaming laptop under $1,200", "Ergonomic chair for a home office"]);
  const startResearch = () => { if (value.trim()) { const prompt = value.trim(); setRecent((items) => [prompt, ...items.slice(0, 2)]); onResearch(prompt); } };
  return <section className="rounded-3xl border border-primary/20 bg-card/80 p-5 shadow-glow sm:p-6">
    ...
    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-2">{examples.map((example) => <button className="rounded-full border border-border px-3 py-1.5 text-left text-xs text-muted-foreground transition hover:border-primary/40 hover:text-primary" key={example} onClick={() => setValue(example)}>{example}</button>)}</div><div className="flex gap-2"><Button onClick={() => setValue("")} size="sm" variant="outline">Clear</Button><Button disabled={loading || !value.trim()} onClick={startResearch} size="sm"><Sparkles className="mr-2 size-4" />{loading ? "Researching…" : "Start AI Research"}</Button></div></div>
