...
export function VerifiedSensoContext({
  context,
  loading = false,
}: {
  ...
  return (
    ...
    >
      ...
      <p
        aria-live="polite"
        className="mt-2 flex items-center gap-2 text-sm leading-6 text-muted-foreground"
      >
        {loading && (
          <LoaderCircle className="size-4 shrink-0 animate-spin text-primary" />
        )}
        {loading
          ? "Verifying merchant with Senso..."
          : unavailable
            ? "Verified context temporarily unavailable."
            : context.groundedAnswer}
      </p>
      {!unavailable && context.citations.length > 0 && (
        <ul className="mt-3 space-y-2">
          {context.citations.map((citation, index) => (
            <li
              className="rounded-lg bg-background/50 px-3 py-2 text-xs"
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {citation.url && (
                  <a
                    aria-label={`Open ${citation.title} source`}
              ...
              <p className="mt-1 text-muted-foreground">
                {citation.relevanceScore !== undefined
                  ...
                  : "Relevance not supplied"}
                {citation.lastVerifiedAt
                  ? ` · Last verified ${citation.lastVerifiedAt}`
                  : ""}
              </p>
            </li>
          ))}
