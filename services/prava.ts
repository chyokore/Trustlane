...
function callbackUrl() { const value = process.env.NEXT_PUBLIC_APP_URL ?? "https://trustlane-pi.vercel.app"; const url = new URL(value); if (url.protocol !== "https:") throw new Error("Hosted checkout requires an HTTPS application URL."); return new URL("/dashboard/checkout/complete", url).toString(); }
