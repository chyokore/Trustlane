...
import { SensoIntegrationCard } from "@/components/senso/integration-card";
...
export default function DashboardPage() {
  return <main className="mx-auto max-w-[1500px] px-4 py-7 sm:px-6 lg:px-8 lg:py-9"><section className="relative overflow-hidden rounded-3xl border border-primary/20 bg-card px-6 py-10 sm:px-10 sm:py-12"><div aria-hidden className="absolute -right-20 -top-28 size-80 rounded-full bg-primary/15 blur-[100px]" /><div className="relative max-w-2xl"><p className="mb-4 text-sm font-semibold text-primary">YOUR TRUSTLAYER</p><h1 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Welcome to TrustLane</h1><p className="mt-4 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg">Shop confidently with AI that explains every decision.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Button asChild><Link href="/dashboard/shop">Start Shopping <ArrowRight className="ml-2 size-4" /></Link></Button><Button asChild variant="outline"><Link href="/dashboard/decision-ledger"><ReceiptText className="mr-2 size-4" />View Decision Ledger</Link></Button></div></div></section><section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((metric) => <MetricCard key={metric.label} metric={metric} />)}</section><section className="mt-7 grid gap-7 xl:grid-cols-[minmax(0,1.65fr)_minmax(280px,0.75fr)]"><RecentActivity items={activity} /><div className="space-y-4"><aside className="rounded-2xl border border-border bg-card/60 p-5"><h2 className="font-semibold">Trust overview</h2><p className="mt-1 text-sm text-muted-foreground">Your commerce activity is protected by transparent review.</p><div className="mt-8"><div className="flex items-end justify-between"><p className="text-3xl font-semibold tracking-tight">94</p><p className="text-xs text-primary">Excellent</p></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full w-[94%] rounded-full bg-primary" /></div><p className="mt-3 text-xs leading-5 text-muted-foreground">Based on merchant verification, approval controls, and decision transparency.</p></div></aside><SensoIntegrationCard /></div></section></main>;
}
